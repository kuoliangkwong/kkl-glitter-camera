...
public class Camera1 extends CameraImpl {
    ...
    private FocusRunnable focusRunnable = new FocusRunnable();
    ...
    @Override
    synchronized void setFacing(@Facing int facing) {
    ...
    @Override
    synchronized void setFlash(@Flash int flash) {
        try {
    ...
    @Override
    synchronized void setFocus(@Focus int focus) {
        this.mFocus = focus;
        switch (focus) {
    ...
    void endVideo() {
        ...
        mMediaRecorder = null;
        mCameraListener.onVideoTaken(file);
    }
    ...
    String getStringFromSizeList(String delimiter, List<Camera.Size> sizes) {
        if (sizes == null) return "NULL";
        String str = "";
    ...
    private int calculatePreviewRotation() {
        if (mCameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            return ((mCameraInfo.orientation) + 360 + 180) % 360;
        } else {
            // return (mCameraInfo.orientation - mDisplayOrientation + 360) % 360;
            return (mCameraInfo.orientation + 360) % 360;
        }
    }
    ...
    private int calculateCaptureRotation() {
        ...
        if (mCameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            return (previewRotation + orientation + 720) % 360;
        } else {
            // return previewRotation;
            return (previewRotation + mDisplayOrientation) % 360;
        }
    }

    private void adjustCameraParameters() {
        initResolutions();
        boolean invertPreviewSizes = false; // boolean invertPreviewSizes = mDisplayOrientation%180 != 0;
    ...
    private void initMediaRecorder() {
        ...
        File parentDirFile = context.getExternalFilesDir(null);
        if (parentDirFile == null) {
            parentDirFile = context.getCacheDir();
        }
        parentDirFile.mkdirs();
        String parentDir = parentDirFile.getAbsolutePath();
        mVideoFile = new File(parentDir, "video.mp4");
        ...
        mPreview.getView().setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    focusRunnable.set(event.getX(), event.getY());
                    mHandler.post(focusRunnable);
                }
                return true;
            }
        });
    ...
    class FocusRunnable implements Runnable {
        ...
        public void run() {
            if (mCamera != null) {
                ...
                Rect rect = calculateFocusArea(mX, mY);
